...
AT(.text.bsp.i2s)
static bool tas5711_detect(void)
{
//    bool ret;
//    bsp_i2c_start();
...
AT(.text.bsp.i2s)
static void tas5711_sfr_write(u8 addr, u8 dat)
{
//    bsp_i2c_start();
//    bsp_i2c_tx_byte(0x36);
//    bsp_i2c_rx_ack();
...
//    bsp_i2c_stop();
//    delay_5ms(2);
    bsp_i2c_tx_byte(0x36, addr, dat);
}

AT(.text.bsp.i2s)
static u8 tas5711_sfr_read(u8 addr)
{
//    u8 dat;
//    bsp_i2c_start();
//    bsp_i2c_tx_byte(0x36);
...
//    return dat;
    u8 buf[1];
    bsp_i2c_rx_buf(0x37 << 8 | 0x36, addr, buf, 1);
    return buf[0];
}
...
void tas5711_init(void)
{
    ...
    tas5711_sfr_write(TAS5711_OSC_TRIM_REG, 0x00);      //tirm Oscillator
    delay_5ms(40);

    for (i=0; i<sizeof(tas5711_cmd_init)/sizeof(tas5711_cmd_t); i++) {
        tas5711_sfr_write(tas5711_cmd_init[i].cmd, tas5711_cmd_init[i].parm);
        delay_us(10);
    }
